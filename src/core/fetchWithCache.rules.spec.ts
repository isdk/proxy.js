...
describe('fetchWithCache Rules & POST', () => {
  ...
  it('默认情况下不应该缓存 POST 请求', async () => {
    ...
    expect(res1.headers.get('x-proxy-cache')).toBe('MISS_EXCLUDED_REQUEST'); // 不走缓存逻辑，所以没有标头
  ...
  it('cacheRules 应该支持多条规则 (OR 逻辑)', async () => {
    ...
    expect(resC.headers.get('x-proxy-cache')).toBe('MISS_EXCLUDED_REQUEST');
  });
  ...
  it('cacheRules 应该支持 Query 布尔匹配 (存在/不存在)', async () => {
    ...
    expect((await fetchWithCache(req2, mockFetcher, { cache, config, activeCacheWrites })).headers.get('x-proxy-cache')).toBe('MISS_EXCLUDED_REQUEST');

    // 3. 不满足条件：有 forbid-me
    const req3 = new Request('https://api.example.com/api?require-me=1&forbid-me=1');
    expect((await fetchWithCache(req3, mockFetcher, { cache, config, activeCacheWrites })).headers.get('x-proxy-cache')).toBe('MISS_EXCLUDED_REQUEST');
  });
